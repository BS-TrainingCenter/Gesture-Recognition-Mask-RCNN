"""
Mask R-CNN
Train on the toy Gesture dataset and implement color splash effect.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 gesture.py train --dataset=/path/to/gesture/dataset --weights=coco
    # Resume training a model that you had trained earlier
    python3 gesture.py train --dataset=/path/to/gesture/dataset --weights=last
    # Train a new model starting from ImageNet weights
    python3 gesture.py train --dataset=/path/to/gesture/dataset --weights=imagenet
    # Apply color splash to an image
    python3 gesture.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>
    # Apply color splash to video using the last weights you trained
    python3 gesture.py splash --weights=last --video=<URL or path to file>
"""
...
class GestureConfig(Config):
    ...
    NUM_CLASSES = 1 + 2  # Background + gesture
...
class GestureDataset(utils.Dataset):
    ...
    def load_gesture(self, dataset_dir, subset):
        """Load a subset of the Gesture dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("hand", 1, "flip")
        self.add_class("hand", 2, "flop")
        ...
        for a in annotations:
            ...
            # the outline of each hand instance. There are stores in the
            ...
            hands = [s['region_attributes'] for s in a['regions'].values()]
            # print("multi_numbers=", multi_numbers)
            # num_ids = [n for n in multi_numbers['number'].values()]
            # for n in multi_numbers:
            num_ids = [int(n['hand']) for n in hands]
